...
partial class HandPipeline
{
    void RunPipeline(Texture input)
    {
        ...
        cs.SetBuffer(0, "_spad_output", _inputBuffer);
        cs.Dispatch(0, InputWidth / 8, InputWidth / 8, 1);

        // Palm detection
        _detector.palm.ProcessImage(_inputBuffer);
        ...
        cs.SetBuffer(1, "_bbox_region", _regionBuffer);
        cs.Dispatch(1, 1, 1, 1);

        // Hand region cropping
        cs.SetTexture(2, "_crop_input", input);
        cs.SetBuffer(2, "_crop_region", _regionBuffer);
        cs.SetBuffer(2, "_crop_output", _cropBuffer);
        cs.Dispatch(2, CropSize / 8, CropSize / 8, 1);

        // Hand landmark detection
        _detector.landmark.ProcessImage(_cropBuffer);
        ...
        cs.SetBuffer(3, "_post_region", _regionBuffer);
        cs.SetBuffer(3, "_post_output", _filterBuffer);
        cs.Dispatch(3, 1, 1, 1);
